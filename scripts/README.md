...
| `switch_top_model.py` | Pick a model **type** (Dense / Hybrid MoE / MoE) and one of the top-5 fastest in that group — ranked live from the OpenCode end-to-end benchmark table — then stop all servers, start the right one, sync the OpenCode config, and smoke-test a tool call. Reuses `chk_llm_macstu.py` + `switch_opencode_config.py`. `--debug` traces every step on stderr (table parse, recipe match, each SSH `rc/stdout/stderr`, readiness polls, smoke payload/response; API key redacted); a `wait_ready` timeout auto-tails the target's remote `/tmp/*.log` even without `--debug`. Recipe paths are written as `{stack}` / `{models}` placeholders and resolved per host via `--stack-dir` / `--models-dir` (or `$MACSTUDIO_STACK_DIR` / `$MACSTUDIO_MODELS_DIR`). |
| `fetch.sh` | Downloads the model weights `switch_top_model.py`'s `LAUNCH_RECIPES` expect but that aren't on the host — GGUF single files and full MLX/HF repos — into `$MODELS_DIR` (LM Studio's `downloadsFolder`), plus the MiniCPM5 HF checkpoint into the HF cache. Idempotent; run with `bash`, not fish. Prune the list to the recipes you actually use. |
| `install_stack.sh` | Builds the runtimes that ship no binary and must be compiled from source — mainline `ggml-org/llama.cpp` (port 8100 recipes: MTP, runtime LoRA, stock GGUF), the TheTom llama.cpp TurboQuant fork (port 8099 `turbo2/3/4` recipes), and SGLang's Apple-Silicon/MLX path with its `sglang-mps` venv (port 30000 recipes) — under one root, `$STACK_DIR` (default `~/Stack`), instead of scattering them in `$HOME`. Applies `patches/patch_sglang_mlx_launch_ts.py` automatically. Idempotent (fetch + reset, not re-clone); `ONLY=mainline\|thetom\|sglang` builds just one. Pass the same root to `switch_top_model.py --stack-dir`. |
...
| [`patches/patch_sglang_mlx_launch_ts.py`](patches/patch_sglang_mlx_launch_ts.py) | SGLang MLX backend `batch.launch_ts` crash — mandatory, the server SIGQUITs on its first request without it. Applied by `install_stack.sh`; re-run after `git pull` in the checkout |
